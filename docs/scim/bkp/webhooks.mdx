...
Webhooks are powerful tools that allow your application to receive real-time
...
By leveraging webhooks, you can build responsive and dynamic integrations that
...
While webhooks are a powerful tool for receiving real-time updates, it's
...
There are typically two types of webhook security mechanisms that you can use to
ensure the authenticity and integrity of the incoming webhook.

### Webhook Signature verification

Every incoming webhook is signed with a secret key. This secret key is available
in the Scalekit Dashboard under the "Webhooks" section.



A cryptographic hash is generated for the payload containing webhook id, webhook
body and the timestamp with the webhook secret as a key using the HMAC-SHA256
algorithm. This signature is then sent as part of the `svix-signature` header.
...
Even though the webhook signature verification is enough to ensure that the
...
To mitigate the risk of replay attacks, we recommend that you verify the webhook
...
Below are the list of Directory Sync webhooks that you can use to listen for
