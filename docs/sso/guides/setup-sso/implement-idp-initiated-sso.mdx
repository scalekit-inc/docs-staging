...
# How to handle Identity Provider (IdP) initiated SSO
...
Fortunately, you can overcome the inherent security risks of IdP initiated SSO by converting the incoming IdP initiated request to an SP initiated SSO flow. Refer the below diagram for the recommended workflow from Scalekit.
...
1. When Scalekit receives an IdP initiated SSO request for one of your customers, it will ping your configured default <SimpleCode>redirect_uri</SimpleCode> with the <SimpleCode>connection_id</SimpleCode> as a request parameter:
