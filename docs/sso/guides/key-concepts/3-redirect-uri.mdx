...
A Redirect URI is an endpoint URL where Scalekit redirects users post-authentication with the Identity Provider (IdP). In the Scalekit dashboard, you can configure up to 5 redirect URIs per environment.
...
**IdP-initiated SAML SSO Flow**: Scalekit uses the Redirect URI that's marked as default to redirect users. [Learn more about handling IdP-initiated SSO flows](/sso/guides/setup-sso/implement-idp-initiated-sso).
...
| Wildcard support (\*)                                                                            | <FcCheckmark /> Allowed<br /><br />See [below](#wildcard-characters-in-redirect-uris) for full details about wildcard character support | <FcCancel /> Not Allowed                       |
