...
Directory sync allows your app to automatically provision and deprovision user accounts based on events from your
customers' directory provider**s.** Implementing directory sync involves three main steps:

1. **Enable Directory Sync**: In the Scalekit console, go to the list of organizations and select the desired
   organization that has the directory provider you want to integrate with.
2. **Configure Directory Sync**: Generate a self-serve admin portal link in the Scalekit console. Provide this link to
   your customer's IT admin so they can configure their directory provider to send events to Scalekit. Scalekit provides
   integration guides for supported providers.
3. **Receive Directory Events**: Create a webhook endpoint and register it in the "Webhooks" section of the Scalekit
   console. Scalekit will send normalized payloads about directory events to this endpoint in real-time, allowing your
   app to take actions like creating or removing user accounts.
...
1. **Switch Environment**: In the Scalekit console, switch to your desired environment.
