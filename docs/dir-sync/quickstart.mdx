...
| Automated User Profile Updates         | scalekit.dir.user.update         | Triggered when a user's information is modified in the directory. <br /><br />For detailed list of directory user profile details, see [here](/dir-sync/events)        |
| Automated User Deprovisioning         | scalekit.dir.user.delete         | Triggered when a user is removed from the directory. <br/><br />Also triggered when a user's status is marked as `inactive` in the directory or if the user is removed from all groups assigned to this application.        |
