...
Scalekit uses Svix to deliver webhooks, so you can use the Svix library to automatically verify incoming webhooks. [Find instructions to install the Svix library](https://docs.svix.com/setup).
...
If your app experiences downtime and misses events sent by Scalekit, you can redeliver those events from the Scalekit Dashboard:
1. Navigate to Webhooks
2. Click the three dots and select "Replay"
3. Choose "Resent all failed message since" to resend all messages your app failed to acknowledge during its downtime
