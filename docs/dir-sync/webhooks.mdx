...
There are typically two types of webhook security mechanisms that you can use to ensure that the incoming webhooks are indeed coming from Scalekit and not from a malicious actor:

### Webhook Signature verification

Every incoming webhook is signed with a secret key. This secret key is available in the Scalekit Dashboard under the "Webhooks" section. Signature is generated using the HMAC-SHA256 algorithm of the webhook body and the secret key. You can use the following code to verify the signature:
